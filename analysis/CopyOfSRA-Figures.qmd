...
boot_mean = function(x,y){
  out = sample(x,size=length(x),replace = T) - sample(y,size=length(y),replace = T)
  out
}
...
# under social fear amplification:
## Ha: cumulative disease spread is lower
# under social fear extinction
## Ha: cumulative disease spread is higher
...
for (row in 1:nrow(disease_rate_and_release_mechanisms)){
  ...
  for (variable in c("percent_infected","peak_infected","epidemic_end","day_max")){
    ...
    if ("Fear Spread via:\nComplex Contagion" %in% d[,unique(experiment)]){
      test_CC <- t.test(form,
                      ...
                      mutate(experiment = factor(experiment,levels = c("Fear Spread via:\nContagion","Fear Spread via:\nComplex Contagion"))))$p.value
...
# Cox PH model for experiments
time_to_event <- dat[step>2,
                     .(epidemic_end = if_else(min(count_disease_I)==0,step[which.min(count_disease_I)],149)), 
                     by=c("experiment","run_number","Disease_Infection_Rate","Release_Mechanism")] %>%
                .[,status:=1*(epidemic_end<149)]
time_to_event[,experiment:=relevel(factor(experiment),ref="Fear Spread via:\nContagion")]


  cox_results <- NULL
  disease_rate_and_release_mechanisms <- unique(time_to_event[,c("Disease_Infection_Rate","Release_Mechanism")])
  for(row in 1:nrow(disease_rate_and_release_mechanisms)){
    disease_infection_rate = disease_rate_and_release_mechanisms[row,Disease_Infection_Rate]
    release_mechanism = disease_rate_and_release_mechanisms[row,Release_Mechanism]
    
  survival_model <- coxph(Surv(epidemic_end, status) ~ experiment,
                          robust=T,
                          data = time_to_event %>% 
                            filter(Disease_Infection_Rate==disease_infection_rate & Release_Mechanism == release_mechanism)
                    )
  model_summary <- summary(survival_model)
  coefficient_estimates <- data.table(
    cox_PH = coef(model_summary)[,"exp(coef)"],
    pval   = coef(model_summary)[,"Pr(>|z|)"],
    conf_int = model_summary$conf.int %>% 
                            as_tibble() %>% 
                            mutate(conf_int = paste0("(",round(`lower .95`,2),", ",round(`upper .95`,2),")")) %>%
                            pull(conf_int),
    experiment = stringr::str_remove(rownames(coef(model_summary)),"experiment")
    ) %>%
    mutate(stars = case_when(pval <1e-3 ~ "***",
                             pval <1e-2 ~ "**",
                             pval <0.05 ~ "*",
                             pval <0.1 ~ ".",
                             pval <1 ~ " ")) %>%
    mutate(Disease_Infection_Rate=disease_infection_rate,
           Release_Mechanism = release_mechanism) %>%
    mutate(cox_label = paste0("Cox PH Coef: ",round(cox_PH,2),stars," ",conf_int)) %>%
    dplyr::select(experiment,Disease_Infection_Rate,Release_Mechanism,cox_label)
  
    cox_results <- bind_rows(cox_results,coefficient_estimates)
  #print(coefficient_estimates)
  prop_hazards_assumption <- cox.zph(survival_model)
  #cat('\n',prop_hazards_assumption$table[,"p"],'\n')
  }
...
labels <- labels %>% 
  ...
  mutate(stars_percent_infected = if_else(is.na(stars_percent_infected),"",stars_percent_infected),
         stars_peak_infected = if_else(is.na(stars_peak_infected),"",stars_peak_infected),
         stars_epidemic_end = if_else(is.na(stars_epidemic_end),"",stars_epidemic_end),
         stars_day_max     = if_else(is.na(stars_day_max),"",stars_day_max)) %>%
  mutate(CI            = paste0(CI,stars_percent_infected),
         peak_infected = paste0(peak_infected,stars_peak_infected),
         epidemic_end  = paste0(epidemic_end,stars_epidemic_end),
         day_max       =paste0(day_max,stars_day_max)) %>%
  # mutate(CI = paste(CI,peak_infected,epidemic_end,day_max,sep='\n')) %>%
  dplyr::select(experiment,Disease_Infection_Rate,CI) %>%
  unique() %>%
  left_join(cox_results,by = c("experiment","Disease_Infection_Rate","Release_Mechanism")) %>%
  mutate(cox_label = if_else(is.na(cox_label),"",cox_label)) %>%
  mutate(CI = paste0(CI,'\n',cox_label))
...
p = p + geom_text(data = labels,
            mapping = aes(label=CI,alpha=1.0),x=100,y=0.85*y.lim,
...
p.spatial = dat.spatial %>% 
  epi_plot(one_disease_rate = F,base_size = 15,txt_size = 6) + 
...
```{r}
p.spatial = dat.spatial %>% 
  epi_plot(base_size = 18, one_disease_rate = T,txt_size = 8) + 
  labs(title="Spatially Clustered") + 
...
cowplot::plot_grid(p.spatial,
                   p.small_world,
                   nrow=2,labels = c("a","b"))

ggsave('../figures/core/Socially-Driven-Fear-Amplification.pdf', height = 8, width = 11.5, device = cairo_pdf)
```
...
p.spatial = dat.spatial %>% 
  epi_plot(one_disease_rate = F,base_size = 15,txt_size = 7) + 
  labs(title="Spatially Clustered") + 
  theme(plot.title = element_text(hjust = 0.5,size = 18))
...
ggsave('../figures/core/Socially-Driven-Fear-Extinction.pdf',height = 8, width = 11.5, device = cairo_pdf)
```
...
for (ns in dat[,unique(percent_nodes_swapped)]){
  ...
  eqn = paste0("β1 = ",coefs[variable=='percent_links_rewired',label],'\n',
               "β2 = ",coefs[variable=='experimentFear Spread via:\nSocial Risk Appraisal',label])
  data_labels = bind_rows(data_labels,tibble(percent_nodes_swapped=ns,label=eqn,experiment="The"))
}
...
p.clustered_fear_increasing = dat %>%
  ...
  geom_text(data = data_labels,mapping = aes(label=label),
            x=0.19,y=20,alpha=1.0,color='black',size=2) +
...
p.clustered_fear_decreasing = dat %>%
  ...
  stat_summary(geom="point",fun = mean,size=0.75) +
  geom_text(data = data_labels,mapping = aes(label=label),
            x=0.19,y=70,alpha=1.0,color='black',size=2) +
  scale_color_manual(values=color_map,name=NULL) +
  plot_theme(base_size = 14) +
  facet_wrap(~percent_nodes_swapped,ncol=1) +
  scale_x_continuous(labels = scales::percent) + 
...
ggsave('../figures/core/SRA-Results-Robust-to-Clustering.pdf', height = 8, width = 11.5, device = cairo_pdf)
```
...
coordinates = fread(net_file,skip=12,nrows=nrows) %>% 
  select(xcor,ycor,disease_state=`disease-state`,
...
agents_infected_under_contagion_but_not_SRA = contagion_tick_by_tick[who %in% data_combined[contagion_vs_SRA=="R-S",name],][disease_state=="I",][,head(.SD,1),by=who] %>%
  select(who,fear_score,ticks=tick_infected) %>%
...
d1 = data_combined %>%
  ...
  select(who=name,xcor,ycor,compare_color,contagion_vs_SRA,initial_fear_score=initial_fear_score.x) %>%
...
cowplot::ggdraw() + cowplot::draw_plot(p)
ggsave('../figures/core/75-Agent-Network.pdf')
knitr::plot_crop('../figures/core/75-Agent-Network.pdf')
...
plot_table %>%
  ...
  select(Contagion,SRA,`Avg Initial Fear Belief`,Color=compare_color) %>%
...
cowplot::plot_grid(
  cowplot::ggdraw() + cowplot::draw_plot(p), cowplot::ggdraw() + cowplot::draw_image(image = '../data/core/States.png'),
  nrow=1
)
ggsave('../figures/core/network.pdf',width = 11.5, height = 8, dev = cairo_pdf)
...
small_world_cc = fread('../data/core/experiment-09-fear-decreasing-small-world-limited-uptake-under-complex-contagion-table.csv',skip = 6) %>% 
  ...
  select(step,experiment,count_disease_I,count_hiding,run_number) %>%
...
small_world_cc %>%
  ...
  scale_color_manual(values = c("Infected"="red","Hiding"="darkgreen"),name=NULL) +
  plot_theme(base_size = 18) +
  geom_label(data = small_world_cc.labels, mapping = aes(label=label),
             label.size = NA, col='black',size=5) +
  labs(x="Tick",y="Agent Count") 

ggsave('../figures/core/Include-SRA-w-Canonical-Mechanisms.pdf',
       height = 8, width = 11.5, device = cairo_pdf)
```
...
dat %>% epi_plot(one_disease_rate = T,base_size = 15,txt_size = 6) +
  facet_grid(Disease_Infection_Rate ~ experiment + Release_Mechanism)
ggsave('../figures/core/Probabilistic-Release-Mechanism.pdf', height = 8, width = 11.5, device = cairo_pdf)
...
data = fread(file = '../data/core/calibrate-prob-release-damping-table.csv', skip = 6) %>%
  ...
  select(run_number,mean_avg_hide_time,prob_release_damping) 
